...
class InternalClient {
  ...
  sendMessage(where, _content, options = {}) {
    ...
    return this.resolver.resolveChannel(where).then(destination => {
      ...
      if (options.file) {
        return this.resolver.resolveFile(options.file.file).then(file => this.apiRequest("post", _Constants.Endpoints.CHANNEL_MESSAGES(destination.id), true, params, {
          name: options.file.name,
          file: file
        }).then(res => destination.messages.add(new _Message2.default(res, destination, this.client))));
      } else {
        return this.apiRequest("post", _Constants.Endpoints.CHANNEL_MESSAGES(destination.id), true, params).then(res => destination.messages.add(new _Message2.default(res, destination, this.client)));
      }
    });
  }
  ...
  updateMessage(msg, _content, options = {}) {
    ...
    return this.apiRequest("patch", _Constants.Endpoints.CHANNEL_MESSAGE(message.channel.id, message.id), true, params).then(res => message.channel.messages.update(message, new _Message2.default(res, message.channel, this.client)));
  }
